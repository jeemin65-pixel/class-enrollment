...
- 유저 생성 시 이름은 실 서비스에서 중복이 있을 수 있어 이메일을 유니크 식별자로 활용했으며, 이메일 중복 검증 로직만 추가
---
## 설계 결정과 이유
...
- 강의 삭제 시 연관된 수강 신청 데이터 처리 미구현 (삭제 전 연관 Enrollment 먼저 삭제하는 등 다른 방법 고려 필요)
- 동일 사용자가 같은 강의에 중복 신청하는 경우에 대한 검증 미구현
---
## AI 활용 범위
